jQuery(function ($) {
    ...
    $('.filter-kategorie select').on('change', function() {
        if( $(this).val() != "") {
            ...
            $('.js-dataTable').DataTable().column(5).search($(this).val().join('|'),true,false).draw();
        } else {
            $('.js-dataTable').DataTable().column(5).search("").draw();
        }
    });
    $('.filter-bereich select').on('change', function() {
        $('.js-dataTable').DataTable().column(4).search($(this).val(),false,true).draw();
    });

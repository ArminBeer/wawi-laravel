<x-app-layout>
    <x-slot name="header">
        ...
        <x-nav-link :href="route('naehrwerte.create')" class="inline-flex items-center h-10 px-5 bg-strizzi">
                        ...
                        <div class="shadow overflow-hidden border-b border-gray-200">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @if (count($naehrwerte) > 0)
                                        @foreach ($naehrwerte as $naehrwert)
                                            <tr>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{$naehrwert['name']}}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{$naehrwert['einheit']}}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap flex justify-end items-center">
                                                    <div class="text-sm text-gray-900">
                                                        <a href="{{ route('naehrwerte.edit', ['naehrwerte' => $naehrwert['id']]) }}" class="inline-flex items-center h-10 px-5 text-white bg-gray-800">
                                                        ...
                                                        {{Form::open(array('route' => array( 'naehrwerte.destroy', $naehrwert['id'] ), 'method' => 'delete', 'style' => 'display:inline', 'onsubmit' => "return confirm('Möchten Sie diesen Nährwert wirklich löschen?')"))}}
                                                        <button type="submit" class="inline-flex items-center h-10 px-5 text-white bg-strizzi-red" >
                                        ...
                                        @endforeach
                                    @else
                                        <td class="px-6 py-4 whitespace-nowrap">{{__('Keine Nährwerte vorhanden!')}}</td>

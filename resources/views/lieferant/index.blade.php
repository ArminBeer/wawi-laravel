<x-app-layout>
    <x-slot name="header">
        ...
        <x-nav-link :href="route('lieferanten.create')" class="inline-flex items-center h-10 px-5 bg-strizzi">
                                    ...
                                    @if (count($lieferanten) > 0)
                                        @foreach ($lieferanten as $lieferant)
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{$lieferant['email']}}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{$lieferant['ansprechpartner']}}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{$lieferant['telefon']}}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap flex justify-center items-center">
                                                    <x-nav-link :href="route('lieferanten.show', ['lieferanten' => $lieferant['id']])" class="inline-flex items-center h-10 px-5 mx-1 text-white bg-strizzi">
                                                    ...
                                                    <x-nav-link :href="route('lieferanten.edit', ['lieferanten' => $lieferant['id']])" class="inline-flex items-center h-10 px-5 mx-1 text-white bg-gray-800">
                                                    ...
                                                    {{Form::open(array('route' => array( 'lieferanten.destroy', $lieferant['id'] ), 'method' => 'delete', 'style' => 'display:inline', 'onsubmit' => "return confirm('Möchten Sie diesen Lieferant wirklich löschen?')"))}}

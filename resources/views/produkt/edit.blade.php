        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm">
                <div class="p-6 bg-white border-b border-gray-200">
                    {!! Form::open(['action' => ['ProduktController@update', $produkt['id']], 'method' => 'POST', 'files' => true]) !!}
                    @csrf
                    @method('PUT')
                    <div class="table w-full">
                        <div class="table-row-group">
                            <div class="table-row pt">
                                ...
                                {{Form::text('produkt_name', $produkt['name'], ['class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline', 'placeholder' => 'Rezeptname'])}}
                            </div>
                        </div>
                        <div class="table-row-group">
                            <div class="mb-4">
                                ...
                                <div class="relative bg-blue-100 flex justify-center items-center">
                                    ...
                                    @if ($produkt['picture'])
                                        <img class="m-2 custom-h-36 absolute top-0 left-0" id="existing_img" src="/storage{{$produkt['picture']}}">
                                    @endif
                        ...
                        <div class="table-row-group">
                            <div class="table-row">
                                ...
                                {{Form::textarea('description_short',  $produkt['zubereitung']['description_short'], ['class' => 'w-full px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline', 'placeholder' => 'Kurzbeschreibung'])}}
                            </div>
                        </div>
                        <div class="table-row-group">
                            <div class="table-row">
                                {{Form::label('description_long', 'Zubereitung Ausführlich Beschreibung')}}
                                {{Form::textarea('description_long', $produkt['zubereitung']['description_long'], ['class' => 'w-full px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline', 'placeholder' => 'Ausführliche Anleitung'])}}
                            </div>
                        </div>
                        <!-- Show Zutaten plus option to add new ones -->
                        <div class="flex flex-col mb-5">
                            <div class="flex pt-5 pb-5 text-xl font-bold">
                                        ...
                                        @foreach ($activeZutaten as $activeZutat)
                                            <div class="col-span-10 grid grid-cols-10 gap-4 zutat-delete" id="active_zutat_form">
                                                <div class="col-span-3 zutat_element">
                                                    {{Form::select('zutaten[]',[null => 'Bitte wählen'] + $zutaten, $activeZutat['zutat'], ['class' => 'produkt_zutat w-full px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline select-2'])}}
                                                </div>
                                                <div class="col-span-3 zutat_element">
                                                    {{Form::number('mengen[]', $activeZutat['menge'], ['step' => 'any', 'class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border border-gray-700 focus:shadow-outline'])}}
                                                </div>
                                                <div class="col-span-2 zutat_element">
                                                    {{Form::select('zutat_einheiten[]',[null => 'Bitte wählen'] + $allEinheiten, $activeZutat['einheit'], ['class' => 'produkt_einheiten w-full px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border focus:shadow-outline'])}}
                                                </div>
                                                <div class="col-span-2 zutat_element">
                                                    {{Form::number('zutat_verluste[]', $activeZutat['verlust'], ['step' => 'any', 'class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border border-gray-700 focus:shadow-outline'])}}
                                                </div>
                                            </div>
                                        @endforeach
                                    @endif
                        ...
                        <div class="table-row-group">
                            <div class="table-row">
                                ...
                                {{Form::select('einheit', $allowedEinheiten, $produkt['produced_zutat']['einheit'], ['id' => 'zutat_einheit', 'class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline'])}}
                            </div>
                        </div>
                        ...
                        <div class="conversion hidden border p-5 my-2" id="conversionForm">
                            ...
                            <div class="flex flex-row justify-between items-center">
                                <div class="w-1/2 mr-2">
                                    {{Form::label('einheit', 'Einheit')}}
                                    {{Form::select('conversionEinheit', [null => "Bitte wählen"] + $grundEinheiten, $produkt['produced_zutat']['conversion_einheit'], ['class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline'])}}
                                </div>
                                <div class="w-1/2 ml-2">
                                    {{Form::label('faktor_name', 'Faktor')}}
                                    {{Form::text('faktor', $produkt['produced_zutat']['faktor'], ['class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline', 'placeholder' => 'Umrechnungsfaktor'])}}
                                </div>
                            </div>
                        </div>
                        <!-- End -->
                        <div class="table-row-group">
                            <div class="table-row">
                                ...
                                {{Form::number('ertrag', $produkt['ertrag'], ['step' => 'any', 'class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border border-gray-700 focus:shadow-outline', 'placeholder' => 'Ertrag nach Zubereitung'])}}
                            </div>
                        </div>
                        <div class="table-row-group">
                            <div class="table-row">
                                {{Form::label('mindestbestand_label', 'Mindestbestand')}}
                                {{Form::number('mindestbestand', $produkt['produced_zutat']['mindestbestand'], ['step' => 'any', 'class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border border-gray-700 focus:shadow-outline', 'placeholder' => 'Erforderliche Mindestmenge'])}}
                            </div>
                        </div>
                        <div class="table-row-group">
                            <div class="table-row">
                                {{Form::label('lagerort', 'Lagerort')}}
                                {{Form::select('lagerort', $lagerorte, $produkt['produced_zutat']['lagerort'], ['class' => 'w-full h-12 px-4 mb-2 text-lg text-gray-700 placeholder-gray-600 border  focus:shadow-outline'])}}
                            </div>
                        </div>
                        <div class="table-row-group my-2">
                            <div class="table-row text-center">
                                {{Form::label('round_label', 'Nur Ganzzahlige Mengen zulassen', ['class' => 'my-2'])}}
                                {{Form::checkbox('round', 1, $produkt['produced_zutat']['round'], ['class ' => 'my-2 ml-2 h-5 w-5 text-blue-600']) }}
                            </div>
                        </div>
                        <div class="table-row-group">
                            <div class="table-row">
                                {!! Form::hidden('zubereitung_id', $produkt['zubereitung']['id']) !!}
